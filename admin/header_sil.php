...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    $header = array_filter($contents, function ($row) {
    ...
    if (isset($_POST['submit'])) {
        $id = $_POST['id'];
        $sql = $conn->prepare("DELETE FROM index_page WHERE id = :id");
        $sql->bindParam(':id', $id, PDO::PARAM_INT);
        $sql->execute();

        header("Location:header_sil.php");
        exit();
    }
}
                                                        ...
                                                        <?php foreach ($header as $head) { ?>
                                                            <form role="form" action="" method="POST" enctype="multipart/form-data">
                                                                <div class="col-md-12 mb-3">
                                                                    <img src="../assets/img/<?php echo $head['content']; ?>" width="100%" alt="">
                                                                </div>
