...
$icindekiler_id = $_GET['id'];
$icindekiler_dil = $_GET['dil'];

$sql = $conn->prepare("SELECT * FROM icindekiler WHERE id = :id");
$sql->bindParam(':id', $icindekiler_id, PDO::PARAM_STR);
$sql->execute();
$content = $sql->fetch(PDO::FETCH_ASSOC);


$icindekiler_name = $content['name'];
$new_img_name = $content['image'];
$urun_id = $content['urun_id'];
$kategori_id = $content['kategori_id'];

$sql = $conn->prepare("SELECT * FROM urunler WHERE dil = :dil AND id <> :urun_id");
$sql->bindParam(':dil', $icindekiler_dil, PDO::PARAM_STR);
$sql->bindParam(':urun_id', $urun_id, PDO::PARAM_STR);
$sql->execute();
...
$sql->execute();
$content2 = $sql->fetch(PDO::FETCH_ASSOC);

if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    if (isset($_POST['urun_yeni']) && !empty($_POST['urun_yeni'])) {
        $urun_id = $_POST['urun_yeni'];
    }
    if (isset($_POST['name']) && !empty($_POST['name'])) {
        $icindekiler_name = $_POST['name'];
    }
    ...
    $sql = $conn->prepare("UPDATE icindekiler SET urun_id = :urun_id, image = :image, name = :name WHERE id = :id");
    $sql->bindParam(':urun_id', $urun_id, PDO::PARAM_INT);
    $sql->bindParam(':image', $new_img_name, PDO::PARAM_STR);
    $sql->bindParam(':name', $icindekiler_name, PDO::PARAM_STR);
    $sql->bindParam(':id', $icindekiler_id, PDO::PARAM_INT);
    $sql->execute();
    ...
    header("Location:icindekiler_sil.php");
    exit();
}
                                ...
                                <div class="content-wrapper">
                                    ...
                                    <div class="content-header">
                                        <div class="container-fluid">
                                            <div class="row mb-2">
                                                <div class="col-sm-6">
                                                    <h1 class="m-0">İçindekiler Güncelle</h1>
                                                </div><!-- /.col -->
                                            </div><!-- /.row -->
                                        </div><!-- /.container-fluid -->
                                    </div>
                                                ...
                                                <div class="card-body mb-3">
                                                    <form role="form" action="" method="POST" enctype="multipart/form-data" style="width: 100%; align-items: center; flex-direction: column; display: flex;">
                                                        ...
                                                        <div class="col-md-12 mb-3">
                                                            <label for="name" class="form-label fw-bold">İçerik Adı: <?php echo $content['name']; ?></label>
                                                            <input name="name" class="form-control form-control-sm" type="text">
                                                        </div>
                                                        <div class="col-md-12 mb-3">
                                                            <select name="urun_yeni" style="width: 25%; height: 32px; margin-bottom: 16px; ">
                                                                <option selected value="<?php echo $content2['id']; ?>"><?php echo $content2['name']; ?></option>
                                                                <?php foreach ($urunler as $urun) { ?>
                                                                    <option value="<?php echo $urun['id']; ?>"><?php echo $urun['name'];  ?></option>
                                                                <?php } ?>
                                                            </select>
                                                        </div>
                                                        <div class="col-md-12 mb-3">
                                                            <label for="img" class="form-label fw-bold">İçindekiler Görselini Değiştir:</label>
