...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (isset($_POST['h_text']) || isset($_POST['h_btn'])) {
        if (isset($_POST['h_text']) && !empty($_POST['h_text'])) {
            $h_text = nl2br($_POST['h_text']);

            $sql = $conn->prepare("DELETE FROM index_page WHERE type = 'header_text' AND dil = :dil");
            $sql->bindParam(':dil', $dil, PDO::PARAM_STR);
            $sql->execute();

            $sql = $conn->prepare("INSERT INTO index_page (content, type, dil) VALUES (:content, 'header_text' , :dil)");
            $sql->bindParam(':content', $h_text, PDO::PARAM_STR);
            $sql->bindParam(':dil', $dil, PDO::PARAM_STR);
            $sql->execute();
        }
        if (isset($_POST['h_btn']) && !empty($_POST['h_btn'])) {
            $h_btn = nl2br($_POST['h_btn']);

            $sql = $conn->prepare("DELETE FROM index_page WHERE type = 'header_btn' AND dil = :dil");
            $sql->bindParam(':dil', $dil, PDO::PARAM_STR);
            $sql->execute();

            $sql = $conn->prepare("INSERT INTO index_page (content, type, dil) VALUES (:content, 'header_btn' , :dil)");
            $sql->bindParam(':content', $h_btn, PDO::PARAM_STR);
            $sql->bindParam(':dil', $dil, PDO::PARAM_STR);
            $sql->execute();
        }
    }
                                                ...
                                                <div class="card-body mb-3">
                                                    ...
                                                    <?php if ($aktif == 1) { ?>
                                                        <div class="header-preview" style="padding-bottom: 25px; ">
                                                            <h2>Header Fotoğraf Ekle</h2>
                                                        </div>
                                                        <form role="form" action="" method="POST" enctype="multipart/form-data">
                                                            <div class="col-md-12 mb-3">
                                                                <label for="img" class="form-label fw-bold">Header Fotoğraf(1710x786)</label>
                                                        ...
                                                        <form role="form" action="" method="POST" enctype="multipart/form-data">
                                                            <div class="col-md-12 mb-3">
                                                                <label for="h_text" class="form-label fw-bold">Header Yazı(<?php echo htmlspecialchars($h_text); ?>)</label>
                                                                <textarea name="h_text" class="form-control form-control-sm" rows="3"></textarea>
                                                            </div>
                                                            <div class="col-md-12 mb-3">
                                                                <label for="h_btn" class="form-label fw-bold">Header Buton(<?php echo $h_btn; ?>)</label>
                                                                <textarea name="h_btn" class="form-control form-control-sm" rows="3"></textarea>
                                                            </div>
                                                            <input type="hidden" name="dil" value="<?php echo $dil; ?>">
                                                            <div class="row d-flex justify-content-end mr-3">
                                                                <input type="submit" name="submit" class="btn btn-primary" value="Kaydet">
